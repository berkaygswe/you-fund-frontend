...
import { 
  ...
  InsufficientCashErrorData,
  PortfolioApiErrorData
...
import { InsufficientCashDialog } from "./InsufficientCashDialog";
...
export function TransactionDialog({
  ...
  // Insufficient Cash 409 Dialog State
  const [insufficientCashData, setInsufficientCashData] = useState<InsufficientCashErrorData | null>(null);
  const [isInsufficientCashOpen, setIsInsufficientCashOpen] = useState(false);
  const [isRetryingAutoDeposit, setIsRetryingAutoDeposit] = useState(false);
  const [lastSubmittedRequest, setLastSubmittedRequest] = useState<CreateTransactionRequest | null>(null);
  ...
  const { searchResults, loading: searchLoading } = useAssetSearch(debouncedSearchTerm, null, 15);
  ...
  const nativeQuoteCurrency = useMemo(() => {
    if (!selectedAsset) return "USD";
    if (selectedAsset.currency) return selectedAsset.currency.toUpperCase();
    const type = selectedAsset.type?.toLowerCase();
    if (type === "fund") return "TRY";
    if (selectedAsset.symbol?.endsWith(".IS") || selectedAsset.symbol?.endsWith(".E")) return "TRY";
    return "USD";
  }, [selectedAsset]);

  const nativeCurrencySymbol = useMemo(() => {
    if (nativeQuoteCurrency === "USD") return "$";
    if (nativeQuoteCurrency === "TRY") return "₺";
    if (nativeQuoteCurrency === "EUR") return "€";
    return nativeQuoteCurrency;
  }, [nativeQuoteCurrency]);
  ...
  const { prices, loading: loadingPrices } = useFetchFundGraph(
    ...
    startDateStr,
    endDateStr,
    selectedAsset ? nativeQuoteCurrency : null
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Exact current ISO timestamp for today; exact user-selected ISO timestamp for historical date
    const finalTransactionDate = isToday 
    ...
    const request: CreateTransactionRequest = {
      ...
      ifRevisionEquals: selectedPortfolio?.revision,
    };
    ...
    try {
      ...
      await createTransaction.mutateAsync({ data: request, idempotencyKey });
      onOpenChange(false);
      resetForm();
    } catch (error: unknown) {
      ...
      if (error instanceof AuthApiError) {
        const errorData = error.data as unknown as PortfolioApiErrorData;

        if (error.status === 409 || errorData?.code === 'INSUFFICIENT_CASH') {
          if (errorData?.code === 'INSUFFICIENT_CASH') {
            setInsufficientCashData(errorData as InsufficientCashErrorData);
            setIsInsufficientCashOpen(true);
            return;
          }

          if (errorData?.code === 'STALE_PORTFOLIO_REVISION') {
            queryClient.invalidateQueries({ queryKey: ['portfolios'] });
            queryClient.invalidateQueries({ queryKey: ['portfolioOverview', Number(portfolioId)] });
            queryClient.invalidateQueries({ queryKey: ['portfolioHoldings', Number(portfolioId)] });
            setCustomErrorMessage(t("staleRevisionNotice"));
            return;
          }

          if (errorData?.code === 'INSUFFICIENT_HOLDINGS') {
            setCustomErrorMessage(t("insufficientHoldingsError"));
            return;
          }
        }

        if (error.status === 422 || errorData?.code === 'MARKET_SETTLEMENT_UNAVAILABLE' || errorData?.code === 'PRICE_UNAVAILABLE') {
          setCustomErrorMessage(t("settlementUnavailableError"));
          return;
        }

        setCustomErrorMessage(errorData?.message || error.message || t("generalError"));
      } else if (error instanceof Error) {
        setCustomErrorMessage(error.message);
      } else {
        setCustomErrorMessage(t("generalError"));
      }
    }
  };

  const handleConfirmAutoDeposit = async () => {
    if (!lastSubmittedRequest || !insufficientCashData) return;

    setIsRetryingAutoDeposit(true);
    setCustomErrorMessage(null);

    const autoFundedRequest: CreateTransactionRequest = {
      ...lastSubmittedRequest,
      funding: {
        mode: 'AUTO_DEPOSIT_SHORTFALL',
        notes: 'Auto top-up for purchase',
      },
      ifRevisionEquals: insufficientCashData.portfolioRevision,
    };

    try {
      const newIdempotencyKey = generateIdempotencyKey();
      await createTransaction.mutateAsync({ 
        data: autoFundedRequest, 
        idempotencyKey: newIdempotencyKey 
      });

      setIsInsufficientCashOpen(false);
      setIsRetryingAutoDeposit(false);
      onOpenChange(false);
      resetForm();
    } catch (error: unknown) {
      console.error("Auto-deposit transaction failed", error);
      setIsRetryingAutoDeposit(false);
      setIsInsufficientCashOpen(false);
      if (error instanceof AuthApiError) {
        const errorData = error.data as unknown as PortfolioApiErrorData;
  ...
  return (
    <>
        ...
        <DialogContent className="sm:max-w-[500px] p-0 overflow-hidden bg-background backdrop-blur-xl border-white/10 shadow-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="p-6 pb-0">
            <DialogTitle className="text-2xl font-bold bg-gradient-to-r from-foreground to-foreground/60 bg-clip-text text-transparent">
              {transactionType === 'BUY' ? t("addPositionTitle") : t("recordSaleTitle")}
            </DialogTitle>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit} className="p-6 pt-4 space-y-5">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2 col-span-2">
                <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t("portfolioLabel")}</Label>
                <Select value={portfolioId} onValueChange={(val) => {
                  setPortfolioId(val);
                  setCustomErrorMessage(null);
                }}>
                  <SelectTrigger className="bg-muted/50 border-white/5 h-11">
                    <SelectValue placeholder={t("selectPortfolioPlaceholder")} />
                  </SelectTrigger>
                  <SelectContent>
                    {portfolios?.map((p) => (
                      <SelectItem key={p.id} value={p.id.toString()}>
                        {p.name} ({p.baseCurrency}) - {t("cashPrefix")}: {p.cashBalance?.toFixed(2) || '0.00'}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              {/* Asset Selection */}
              <div className="space-y-2 col-span-2">
                <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t("assetLabel")}</Label>
                {selectedAsset ? (
                  <div className="flex items-center justify-between p-3 rounded-lg bg-primary/5 border border-primary/20 group">
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        className="h-7 px-2.5 text-[10px] flex items-center gap-1 border-primary/20 hover:bg-primary/5 hover:text-primary transition-all active:scale-[0.98] font-bold cursor-pointer"
                      >
                        <span>📈</span>
                      ...
                      {!fixedAsset && (
                        <Button 
                          ...
                          className="h-7 text-xs opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
                        >
                          {t("changeAsset")}
                        </Button>
                      )}
                      ...
                      <div className="absolute top-full left-0 right-0 mt-2 z-50 rounded-xl border border-white/10 bg-background/95 backdrop-blur-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        <div className="max-h-60 overflow-y-auto overscroll-contain">
                          {searchLoading ? (
                            ...
                            <div className="p-2 space-y-1">
                              {searchResults.map((asset) => (
                                <button
                                  ...
                                  className="w-full flex items-center justify-between p-3 rounded-lg hover:bg-white/5 transition-colors text-left cursor-pointer"
                                >
                                  <div className="flex items-center gap-3">
                            ...
                            <div className="p-8 text-center text-xs text-muted-foreground">{t("noAssetsFound")}</div>
                          )}
                        </div>
                      </div>
                    )}
              ...
              <div className="space-y-2">
                <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t("quantityLabel")}</Label>
                <Input 
                  type="number" 
                  step="any"
                  placeholder="0.00" 
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t("pricePerUnitLabel")}</Label>
                  {selectedAsset && (
                    <Badge variant="outline" className="text-[10px] font-mono px-1.5 py-0 border-white/10 text-muted-foreground">
                      {nativeQuoteCurrency}
                    </Badge>
                  )}
                </div>
                <div className="relative">
                  ...
                  <Input 
                    ...
                    step="any"
                    placeholder="0.00" 
                    className="pl-7 pr-12 h-11 bg-muted/50 border-white/5"
                    value={pricePerUnit}
                    onChange={(e) => {
                  ...
                  />
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] font-semibold text-muted-foreground font-mono pointer-events-none">
                    {nativeQuoteCurrency}
                  </span>
                </div>
              </div>
                ...
                <div className="space-y-4 col-span-2 p-4 rounded-xl bg-muted/30 border border-white/5">
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-muted-foreground font-medium">
                      {t("historicalRange", { 
                        ...
                        range: `${nativeCurrencySymbol}${bounds.low.toFixed(2)} - ${nativeCurrencySymbol}${bounds.high.toFixed(2)}` 
                      })}
                    </span>
                    {activeDayPrice && (
                      <button
                        ...
                        className="text-primary hover:text-primary/80 transition-colors text-xs font-semibold cursor-pointer"
                      >
                        {t("useClosePrice", { price: `${nativeCurrencySymbol}${bounds.close.toFixed(2)}` })}
                      </button>
                    )}
                  </div>
                  ...
                  <div className="relative pt-6 pb-2">
                    <div className="absolute top-0 left-0 text-[10px] text-emerald-500/80 font-semibold font-mono">
                      {t("lowLabel")} ({nativeCurrencySymbol}{bounds.low.toFixed(2)})
                    </div>
                    <div className="absolute top-0 right-0 text-[10px] text-emerald-500/80 font-semibold font-mono">
                      {t("highLabel")} ({nativeCurrencySymbol}{bounds.high.toFixed(2)})
                    </div>
                    ...
                    {(() => {
                      ...
                      const percent = bounds.range > 0 ? Math.min(Math.max(((val - bounds.min) / bounds.range) * 100, 0), 100) : 50;
                  ...
                  <div className="flex justify-between items-center text-[10px] text-emerald-500 font-semibold font-mono mt-4 px-1">
                    <span>{t("lowLabel")}: {nativeCurrencySymbol}{bounds.low.toFixed(2)}</span>
                    <span className="text-muted-foreground">{t("closeLabel")}: {nativeCurrencySymbol}{bounds.close.toFixed(2)}</span>
                    <span>{t("highLabel")}: {nativeCurrencySymbol}{bounds.high.toFixed(2)}</span>
                  </div>
              ...
              {/* Date */}
              <div className="space-y-2">
                <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t("dateLabel")}</Label>
                <DateTimePicker 
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t("feesLabel")}</Label>
                  {selectedAsset && (
                    <Badge variant="outline" className="text-[10px] font-mono px-1.5 py-0 border-white/10 text-muted-foreground">
                      {nativeQuoteCurrency}
                    </Badge>
              ...
              {/* Notes */}
              <div className="space-y-2 col-span-2">
                <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t("notesLabel")}</Label>
                <Input 
              ...
              {total !== null && (
                <div className="col-span-2 p-4 rounded-xl bg-primary/5 border border-primary/20 space-y-2 animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="flex justify-between items-center text-xs text-muted-foreground font-medium">
                    ...
                    <span>
                      {nativeCurrencySymbol}{total.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 6 })} <span className="font-mono text-[10px] opacity-75">{nativeQuoteCurrency}</span>
                    </span>
                  </div>
                  {parseFloat(fee) > 0 && (
                    <div className="flex justify-between items-center text-xs text-muted-foreground font-medium">
                      ...
                      <span>
                        {transactionType === "BUY" ? "+" : "-"} {nativeCurrencySymbol}{parseFloat(fee).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 6 })} <span className="font-mono text-[10px] opacity-75">{nativeQuoteCurrency}</span>
                      </span>
                    </div>
                  )}
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-lg font-bold text-primary">
                      {nativeCurrencySymbol}{totalWithFees?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 6 })} <span className="text-xs font-mono text-muted-foreground">{nativeQuoteCurrency}</span>
                    </span>
                  </div>
                </div>
              )}
            ...
            <DialogFooter className="pt-2">
              ...
              >
                ...
                {transactionType === 'BUY' ? t("submitBuy") : t("submitSell")}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
      ...
      <InsufficientCashDialog
        open={isInsufficientCashOpen}
        onOpenChange={setIsInsufficientCashOpen}
        errorData={insufficientCashData}
        onConfirmAutoDeposit={handleConfirmAutoDeposit}
        isRetrying={isRetryingAutoDeposit}
      />
    </>
  );
}
