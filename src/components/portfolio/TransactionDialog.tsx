  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error instanceof AuthApiError) {
        ...
        if (error.status === 422 || errorData?.code === 'MARKET_SETTLEMENT_UNAVAILABLE') {
          setCustomErrorMessage(errorData?.message || t("settlementUnavailableError"));
          return;
        }
        ...
        <DialogContent className="sm:max-w-[500px] p-0 overflow-hidden bg-background backdrop-blur-xl border-white/10 shadow-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="p-6 pt-4 space-y-5">
            <div className="grid grid-cols-2 gap-4">
              ...
              {/* Currency Selection */}
              <div className="space-y-2">
                <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t("currencyLabel")}</Label>
