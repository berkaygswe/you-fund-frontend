        ...
        <div className="min-h-screen w-full pb-12">
            ...
            <div className="relative z-10 flex flex-col space-y-6 w-full max-w-[1600px] mx-auto pt-2">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    {[
                        { label: "Total Assets", val: "1,247", change: "+12.5%", icon: Globe, color: "text-blue-500" },
                        { label: "Active Markets", val: "24", change: "+2", icon: Activity, color: "text-indigo-500" },
                        { label: "24h Volume", val: "$2.4B", change: "+8.2%", icon: DollarSign, color: "text-emerald-500" },
                        { label: "Market Sentiment", val: "BULLISH", change: "65%", icon: LineChart, color: "text-amber-500" }
                    ].map((stat, i) => (
                        <Card key={i} hoverable className="p-0 border-none sm:border">
                            <CardContent className="p-6 flex flex-col justify-between h-full">
                                <div className="flex justify-between items-start mb-4">
                                    <span className="text-[10px] font-bold tracking-[0.15em] text-muted-foreground uppercase">{stat.label}</span>
                                    <stat.icon className={`h-4 w-4 ${stat.color}/80`} />
                                </div>
                                <div>
                                    <div className={`text-2xl font-mono font-bold tracking-tight ${stat.label === 'Market Sentiment' ? 'text-emerald-500' : 'text-foreground'}`}>
                                        {stat.val}
                                    </div>
                                    <div className="flex items-center mt-2 text-[10px] font-mono font-bold">
                                        <span className={stat.label === 'Market Sentiment' ? 'text-foreground/60' : 'text-emerald-500 flex items-center bg-emerald-500/10 px-1.5 py-0.5 rounded-sm'}>
                                            {stat.label !== 'Market Sentiment' && <TrendingUp className="h-3 w-3 mr-1" />} {stat.change}
                                        </span>
                                        <span className="text-muted-foreground/40 ml-2 uppercase tracking-tighter">
                                            {stat.label === 'Market Sentiment' ? 'positive metrics' : 'vs last period'}
                                        </span>
                                    </div>
                                </div>
                            </CardContent>
                        ...
                        <div className="flex flex-col space-y-3 pt-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-1 gap-2">
                                {[
                                    ...
                                    { href: "/compare", icon: Search, label: "Compare Assets" },
                                    { href: "/calendar", icon: Calendar, label: "Market Calendar" }
                                ].map((item, idx) => {
                                    const Content = (
                                        <>
                                    ...
                                    const className = "group flex items-center justify-between p-3.5 rounded-xl border border-border/20 bg-card/30 hover:bg-card/80 transition-all backdrop-blur-sm cursor-pointer relative overflow-hidden text-left";
